    ...
    <main className="min-h-screen bg-stone-50 text-stone-900">
      ...
      <section className="border-t border-stone-200 bg-white">
        <div className="mx-auto max-w-7xl px-6 py-14 lg:px-8">
          <h2 className="text-3xl font-bold text-stone-950">Conditions at a Glance</h2>
            ...
            <table className="w-full min-w-[720px] text-left text-sm">
              ...
              <tbody>
                {conditions.map((condition) => (
                  <tr key={condition.slug} className="border-t border-stone-200 odd:bg-white even:bg-stone-50">
                    <td className="px-4 py-3 font-semibold">
                      <Link href={`/conditions/${condition.slug}`} className="text-sage-700 hover:underline">
                        {condition.title}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-stone-600">{condition.category}</td>
                    <td className="px-4 py-3 text-stone-600">{condition.symptoms[0]}</td>
      ...
      {groups.map((group) => (
        <section key={group.category} className="border-t border-stone-200 bg-stone-50">
          <div className="mx-auto max-w-7xl px-6 py-14 lg:px-8">
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {group.items.map((condition) => (
                ...
                >
                  <h3 className="text-2xl font-bold leading-snug text-stone-950">
                    {condition.title}
                  </h3>
                  <p className="mt-3 leading-7 text-stone-600">{condition.shortDescription}</p>
                  <p className="mt-5 font-semibold text-sage-700">Read full guide</p>
                </Link>
              ))}
            </div>
          </div>
