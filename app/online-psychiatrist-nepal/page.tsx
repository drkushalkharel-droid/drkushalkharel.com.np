...
function Section({ title, children }: { title: string; children: React.ReactNode }) { return <section className="rounded-lg border border-stone-200 bg-white p-6 shadow-sm md:p-8"><h2 className="text-3xl font-bold text-sage-950">{title}</h2><div className="mt-5 space-y-4 text-lg leading-9 text-stone-700">{children}</div></section>; }
function Resource({ href, children }: { href: string; children: React.ReactNode }) { return <Link href={href} className="rounded-lg border border-stone-200 p-4 font-semibold text-sage-800 transition hover:border-sage-300 hover:bg-sage-50">{children}</Link>; }
