...
import type { ScreeningTool } from "../data/screening";
...
function getInterpretation(tool: ScreeningTool, score: number) {
  return (
    tool.interpretations.find(
      (interpretation) => score >= interpretation.min && score <= interpretation.max,
    ) ?? tool.interpretations[tool.interpretations.length - 1]
  );
}

export default function SingleScreeningTool({ tool }: { tool: ScreeningTool }) {
  const [scores, setScores] = useState<Scores>(() => initialScores(tool));

  const totalScore = Object.values(scores).reduce((sum, value) => sum + value, 0);
  const interpretation = getInterpretation(tool, totalScore);
  const hasSelfHarmSignal = tool.items.some(
    (item) => item.flagsSelfHarm && (scores[item.id] ?? 0) > 0,
  );

  function updateScore(itemId: string, value: number) {
    setScores((current) => ({ ...current, [itemId]: value }));
  }

  function reset() {
    setScores(initialScores(tool));
  }
